        ...
        <div className="page-header-row">
          <div>
            ...
            <p style={{ fontSize: 13.5, color: 'var(--text-muted)' }}>
              {totalMsgs} mensagens · {campaigns?.length || 0} campanhas
            </p>
          </div>
        ...
        {!campaigns || campaigns.length === 0 ? (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: 14 }}>
            {campaigns.map(c => {
              ...
              return (
                <div key={c.id} className="card" style={{ padding: 18, display: 'flex', flexDirection: 'column', gap: 12 }}>
                  ...
                  {c.message_text && (
                    <p style={{
                      ...
                      background: '#f9fafb', borderRadius: 8, padding: '9px 11px', margin: 0,
                  ...
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: 6 }}>
                    <span style={{ fontSize: 28, fontWeight: 800, letterSpacing: '-0.03em' }}>{c.sent_to_count}</span>
                    <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>msgs enviadas</span>
                  </div>
                  <span style={{
                    fontSize: 11.5, fontWeight: 600, padding: '2px 8px', borderRadius: 99, alignSelf: 'flex-start',
